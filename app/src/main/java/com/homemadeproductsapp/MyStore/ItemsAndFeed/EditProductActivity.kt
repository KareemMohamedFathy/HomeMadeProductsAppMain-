...
class EditProductActivity : AppCompatActivity(), OnOptionClickListener,AdapterView.OnItemSelectedListener {
...
// Create a new ImageView and set it's properties
    ...
    private fun setupToolbarText() {
        if (supportActionBar != null) {
            ...
            textViewTitle.setText("Edit Your Product")
    ...
    private fun getIntentData() {
        ...
        val p = intent.getStringExtra("product")

        product = Gson().fromJson<Product>(p, type)
        editTextCopies.text = product.copies.toString()
        editTextDescription.text = product.description
        editTextName.text = product.name
        editTextPrice.text = product.price.toString()
        val imageUri:ArrayList<Uri?> =ArrayList()
        for(cat in product.uriPaths!!){
            imageUri.add(cat.toUri())
        }
        images=imageUri
        Glide.with(this).load(imageUri!![0]).skipMemoryCache(false).placeholder(circularProgressDrawable).into(imageViewAddItem.currentView as ImageView)

        //   imageViewAddItem.set
    ...
    private fun setupSpinners() {
        ...
        for (i in 0 until adapter.count) {
            if (spin.getItemAtPosition(i).equals(category)) {
                spin.setSelection(i)
                break
            }
        }
    ...
    private fun setupClickListeners() {
        val clickAction=object : View.OnClickListener{
            override fun onClick(v: View?) {
                ...
                if (images!!.size==0) {
                ...
                else    if (TextUtils.isEmpty(name)|| TextUtils.isEmpty(price)|| TextUtils.isEmpty(
                        description
                    )) {
                    Toast.makeText(
                        this@EditProductActivity,
                    ...
                    Toast.makeText(
                        this@EditProductActivity,
                        "Max number of character for store name is 30",
                        Toast.LENGTH_SHORT
                    ).show()
                }
                    ...
                    Toast.makeText(
                        this@EditProductActivity,
                        "Max number of character for store description is 150",
                        Toast.LENGTH_SHORT
                    ).show()
                }
                    ...
                    dbReference.child(productId.toString()).setValue(p)

                    intent = Intent(this@EditProductActivity, MyStoreActivity::class.java)
                    startActivity(intent)
                    finish()
        ...
        nextBtn.setOnClickListener {
            ...
            if (position < images!!.size-1){
                position++
                Glide.with(this).load(images!![position].toString()).skipMemoryCache(false).placeholder(
                    circularProgressDrawable
                ).into(imageViewAddItem.currentView as ImageView)
...
//                        images!!.add(imageUri)
    ...
    override fun onBackPressed() {
        super.onBackPressed()
        val intent: Intent = Intent(this@EditProductActivity, MyStoreActivity::class.java)
        startActivity(intent)
        finish()
    }
                    ...
                    .addOnSuccessListener(
                        OnSuccessListener<UploadTask.TaskSnapshot> { taskSnapshot ->
                            taskSnapshot.storage.downloadUrl.addOnSuccessListener {
                                ...
                                Glide.with(this).load(images!![0].toString()).skipMemoryCache(
                                    false
